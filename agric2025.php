<?php
// AGRIC 2025 CBT - Standalone PHP Page
// Data source: agric2024.json (expected to be a top-level array of questions)

// --- Configuration ---
$jsonFile = __DIR__ . '/agric2025.json';
$subjectTitle = 'Agric 2025'; // Corrected title
...
<head>
    ...
    <style>
        /* CSS remains consistent with user requirements */
        ...
        .explanation-btn:hover { background: #005bb5; }
...
<script>
    ...
    async function loadQuestions() {
        try {
            ...
            showError('Network Error: Could not load questions.');
        }
    }
    ...
    function showError(message) {
        ...
        document.getElementById('info-box').innerHTML = 'Error';
        document.getElementById('nav-buttons').style.display = 'none';
        document.getElementById('q-nav').style.display = 'none';
        document.getElementById('section-display').style.display = 'none';
    }
        ...
        if (!explanationData) {
             try {
                 ...
                 if (data.success) {
                     ...
                     explanationContainer.innerHTML = '<div class="error">Failed to load data.</div>';
                 }
             } catch (error) {
                 explanationContainer.innerHTML = '<div class="error">Network Error.</div>';
             }
        } else {
            displayExplanations();
        }
    ...
    function displayExplanations() {
        ...
        explanationData.forEach((qObj) => {
            ...
            qObj.options.forEach(opt => {
                let labelClass = '';
                if (opt.optionId === qObj.correctAnswer) labelClass = 'correct-answer-label';
                else if (opt.optionId === userAnswer) labelClass = 'user-answer-label';
        ...
        html += '<button class="nav-btn" onclick="exportPerformance()">Export Data</button>';
    ...
    // New Export function specifically for Agric data structure
    function exportPerformance() {
        ...
        a.href = url;
        a.download = 'Agric2025_Performance.json';
