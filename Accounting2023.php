<?php
// accounting2023.php - Vector Learn CBT Page (Server-Side Logic)

// Define the JSON file name
$jsonFile = __DIR__ . '/accounting2023.json';
...
function flattenQuestions($data, $includeAnswers = false) {
    ...
    foreach ($source_questions as $question) {
        ...
        // Use 'number' for Accounting JSON structure, or 'questionId' for others
        $qId = $question['number'] ?? $question['questionId'];
        ...
        if (is_array($options_raw) && !empty($options_raw) && isset($options_raw[0]) && is_array($options_raw[0])) {
            ...
            $options = $options_raw;
             ...
             // Case 2: Object format (Your JSON structure)
             // Convert {"A": "text", "B": "text"} into [{"optionId": "A", "text": "text"}, ...]
        ...
        if ($includeAnswers) {
            // Check 'answer' key (used in Accounting JSON) then 'correctAnswer' (used in CRK JSON)
...
if ($action === 'get_questions') {
    ...
    echo json_encode([
        ...
        'subjectName' => $data['subject'] ?? 'Accounting 2023',
...
<?php include 'topnavbar.php'; ?>
    <div class="container">
        ...
        <div class="title">
            <h1>Vector Learn — <span id="exam-title">Accounting 2023</span></h1>
    ...
    async function loadQuestions() {
        try {
            ...
            if (data.success && data.questions.length > 0) {
                ...
                showError('Failed to load questions. Check that accounting2023.json exists, is valid, and contains questions. (Server responded: ' + JSON.stringify(data) + ')');
            }
        } catch (error) {
