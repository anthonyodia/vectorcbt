<?php
// Physics 2024 CBT - Standalone PHP Page
// This file handles both loading and serving the CBT

// CRITICAL: Define the path to the JSON data file.
// NOTE: This now points to the new physics2024.json file.
$jsonFile = __DIR__ . '/physics2024.json'; 
...
function loadJsonData($file) {
    ...
    if (is_array($data) && isset($data[0]['id'])) {
        ...
        // Use the array inside the 'questions' key
        ...
        if (is_array($rawArray) && isset($rawArray[0]['id'])) {
            ...
            return ['error' => 'JSON format is invalid. Ensure it contains a root array of question objects, or an object with a "questions" key containing the array.'];
        }
    }
    ...
    foreach ($questionsArray as $q) {
        // Note: I'll use a generic "Physics" section name since the data didn't provide specific sections.
        $q['sectionName'] = $q['sectionName'] ?? "Core Physics"; 
        $q['sectionId'] = $q['sectionId'] ?? "PHY"; 
...
<head>
    ...
    <style>
        ...
        .question-image-wrapper {
             text-align: center; 
             margin-bottom: 20px;
             /* Ensure images don't stretch the container too much vertically */
             max-height: 40vh; 
             overflow: hidden;
        }
...
<body>
    <div class="container">
        ...
        <div class="title">
            <h1>Vector Learn — Physics 2024</h1>
        ...
        <div class="question-container" id="q-container">
            <div class="error">
                **CRITICAL JSON ERROR:** The quiz cannot load.
                ...
                <p>Please check your <code>physics2024.json</code> file for syntax issues.</p>
            </div>
        </div>
        <?php else: ?>
...
<script>
    ...
    async function loadQuestions() {
        try {
            // Note: PHP_SELF correctly points to physics2024.php now
    ...
    function renderQuestion() {
        ...
        // Since the JSON doesn't define sections, it will default to "Core Physics"
        ...
        html += '</div>';
         ...
         if (qObj.image) {
             html += `<div class="question-image-wrapper">
                        <img src="${htmlEscape(qObj.image)}" alt="Question Diagram" loading="lazy">
                    </div>`;
        }
