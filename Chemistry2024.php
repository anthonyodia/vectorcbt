<?php
// CHEMISTRY 2024 CBT - Standalone PHP Page
// Data source: chemistry2024.json (expected to be a top-level array of questions or an object with a 'questions' key)

// --- Configuration ---
// NOTE: This script requires a file named 'chemistry2024.json' in the same directory.
$jsonFile = __DIR__ . '/chemistry2024.json'; 
$subjectTitle = 'Chemistry 2024'; 
$subjectTagline = 'Exploring the elements and reactions, one question at a time.'; // New descriptive text
...
function prepare_exam_questions($raw_questions) {
    ...
    foreach ($raw_questions as $q) {
        ...
        $question = [
            ...
            'table' => $q['table'] ?? null,
            'options' => [],
            'sectionName' => $q['sectionName'] ?? 'General Chemistry', // Changed default section name
...
<script>
    ...
    // --- Navigation and Answer Handling (Unchanged) ---
    ...
    // Event listeners for navigation (Unchanged)
    ...
    // Immediate update on option selection (Unchanged)
    ...
    // --- API Calls and Views (Unchanged logic, minor path change for error message) ---
    ...
    async function loadQuestions() {
        try {
            ...
            if (data.success && data.questions.length > 0) {
                ...
                showError(data.error || 'Failed to load questions or question bank is empty. Check if chemistry2024.json exists and is valid.'); // Updated file name in error
            }
        } catch (error) {
