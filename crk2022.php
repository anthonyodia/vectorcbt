...
// *** CRITICAL CHANGE: Updated file name to crk2022.json ***
$jsonFile = __DIR__ . '/crk2022.json';
...
if ($action === 'get_questions') {
    ...
    // *** ROBUST PARSING LOGIC: Checks for the specific object structure you provided ***
    if (isset($decodedData['questions']) && is_array($decodedData['questions'])) {
        $fullData = $decodedData['questions']; // Handles the provided WAEC object structure
    } elseif (is_array($decodedData)) {
        $fullData = $decodedData; // Handles a bare array structure (fallback)
    } else {
        http_response_code(500);
        echo json_encode(['error' => 'Invalid JSON format. Expected an object with a "questions" key or a bare array.']);
        exit();
    }
    ...
    $questions = [];
    // *** CRITICAL CHANGE: Updated Section Name ***
    $sectionName = 'Christian Religious Studies (CRS)'; 

    foreach ($fullData as $questionData) {
        // NOTE: The model JSON uses 'text' for the question, not 'question'. Adjusted key here.
        $question = [
            'questionId' => $questionData['id'] ?? null,
            'question' => $questionData['text'] ?? 'Missing Question Text', 
            'options' => [],
            'sectionName' => $sectionName,
            'sectionId' => 1,
            'answer' => $questionData['answer'] ?? '',
            'explanation' => $questionData['explanation'] ?? 'No explanation provided.',
            'imageUrl' => $questionData['image'] ?? null, // Added support for a potential image URL
        ];

        // The JSON options are already structured as an associative array ('A' => 'Rubber', etc.)
        foreach ($questionData['options'] as $optionId => $optionText) {
            $question['options'][] = [
                'optionId' => $optionId, // 'A', 'B', 'C', 'D'
                'text' => $optionText
            ];
        }
        $questions[] = $question;
...
<head>
    ...
    <title>Vector Learn — Christian Religious Studies</title>
    <style>
...
<?php include 'topnavbar.php'; ?>
    <div class="container">
        ...
        <div class="title">
            <h1>Vector Learn — C.R.K 2022</h1>
            <p>Answer the questions carefully by checking the boxes</p>
        </div>
        ...
        <div id="section-display">
            <div class="section-divider">Subject: Christian Religious Studies (CRS)</div>
        </div>
...
<script>
    ...
    let duration = parseInt(urlParams.get('duration')) || 60; // 60 minutes for a 50-question exam
    ...
    async function loadQuestions() {
        try {
            ...
            if (data.success && data.questions.length > 0) {
                ...
                // Pre-populate answers for demonstration/consistency, though normally this is empty
                allQuestions.forEach(q => {
                     // Temporary correction for Q32 logic error (for a real test, this should be corrected in the JSON)
                     if (q.questionId == 32) q.answer = 'B'; 
                });
                ...
                showError('Failed to load questions: ' + (data.error || 'The file might be empty or in an unexpected format.'));
            }
        } catch (error) {
    ...
    function renderQuestion() {
        ...
        // Check for and display image if URL exists
        if (qObj.imageUrl) {
            html += `<img src="${htmlEscape(qObj.imageUrl)}" alt="Diagram for question ${qObj.questionId}" class="question-image" />`;
        }
