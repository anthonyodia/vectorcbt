<?php
// accounting2022.php - Vector Learn CBT Page (Server-Side Logic)

// Define the JSON file name
$jsonFile = __DIR__ . '/accounting2022.json';
...
if ($action === 'get_questions') {
    ...
    echo json_encode([
        ...
        'subjectName' => $data['subject'] ?? 'Accounting 2022',
...
<head>
    ...
    <title>Vector Learn — Accounting 2022 CBT</title>
    <style>
...
<?php include 'topnavbar.php'; ?>
    <div class="container">
        ...
        <div class="title">
            <h1>Vector Learn — <span id="exam-title">Accounting 2022</span></h1>
...
<script>
    ...
    async function loadQuestions() {
        try {
            // Note: The script uses its own filename (e.g., accounting2022.php) for AJAX calls
            ...
            if (data.success && data.questions.length > 0) {
                ...
                showError('Failed to load questions. Check that **accounting2022.json** exists, is valid, and contains questions. (Server responded: ' + JSON.stringify(data) + ')');
            }
        } catch (error) {
    ...
    function displayExplanations() {
        ...
        explanationData.forEach((qObj) => {
            ...
            html += '<span class="question-id">Q' + qId + ' — ' + (isCorrect ? 'Correct ✅' : (userAnswer ? 'Incorrect ❌' : 'Unanswered ❓')) + '</span>';
            html += '<span class="question-section">' + htmlEscape(qObj.sectionName) + '</span>';
            html += '</div>';
