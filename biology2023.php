<?php
// BIOLOGY 2023 CBT - Standalone PHP Page
// Data source: biology2023.json (expected to be a top-level array of questions)

// --- Configuration ---
// NOTE: This script requires a file named 'biology2023.json' in the same directory.
$jsonFile = __DIR__ . '/biology2023.json'; 
$subjectTitle = 'Biology 2023'; 
...
function load_json_data($file) {
    ...
    // Check for array structure: either top-level array or array inside 'questions' key
    if (is_array($data) && isset($data['questions']) && is_array($data['questions'])) {
        ...
        echo "Error: Invalid JSON format. Expected an array of questions. PHP JSON Error: " . $json_error;
        exit();
    }
...
function prepare_exam_questions($raw_questions) {
    ...
    foreach ($raw_questions as $q) {
        ...
        $question = [
            ...
            'table' => $q['table'] ?? null,
            'options' => [],
            'sectionName' => $q['sectionName'] ?? 'General Biology', // Updated default section
...
<?php include 'topnavbar.php'; ?>
    <div class="container">
        ...
        <div class="title">
            ...
            <p>Exploring the wonders of life, one question at a time.</p>
        </div>
...
<script>
    ...
    function renderTable(tableData) {
        ...
        // It's a simple rendering function for the table structure observed in the previous JSON context.
        ...
        headers.forEach(key => {
            ...
            // Simple currency/number formatting example (adjust as needed for biology context)
    ...
    async function loadQuestions() {
        try {
            ...
            if (data.success && data.questions.length > 0) {
                ...
                showError(data.error || 'Failed to load questions or question bank is empty. Check if biology2023.json exists and is valid.');
            }
        } catch (error) {
