<?php
// BIOLOGY 2025 CBT - Standalone PHP Page
// Data source: biology2025.json

// --- Configuration ---
$jsonFile = __DIR__ . '/biology2025.json'; 
$subjectTitle = 'Biology 2025'; 
...
function prepare_exam_questions($raw_questions) {
    ...
    foreach ($raw_questions as $q) {
        ...
        $question = [
            ...
            'table' => $q['table'] ?? null,
            'options' => [],
            'sectionName' => $q['sectionName'] ?? 'General Biology',
...
<?php include 'topnavbar.php'; ?>
    <div class="container">
        ...
        <div class="question-container" id="q-container"><p style="text-align:center;">Loading questions...</p></div>
...
<script>
    ...
    function renderQuestion() {
        ...
        if (qObj.image) html += `<img src="${htmlEscape(qObj.image)}" />`;
    ...
    function displayResults(result) {
        ...
        resView.innerHTML = `<div style="font-size: 32px; font-weight: bold;">Biology Results</div><div style="font-size: 48px; color: #007aff; margin: 20px 0;">${result.score}/${result.total}</div><div style="font-size: 24px; color: #43e97b; margin-bottom: 20px;">${result.percentage}%</div><div class="nav-button-group"><button class="nav-btn explanation-btn" style="background:#007aff" onclick="viewExplanations()">View Explanations</button><button class="nav-btn" onclick="location.reload()">Retake Exam</button></div>`;
    ...
    function exportPerformance() {
        ...
        a.download = 'Biology2025_Performance.json';
        a.click();
    }
