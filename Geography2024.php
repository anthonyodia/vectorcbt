<?php
// geography2024.php - Standalone PHP Page for Geography CBT
// NOTE: Ensure there are absolutely NO spaces or characters before this opening tag.

// UPDATED: Set the correct JSON file path for Geography
$jsonFile = __DIR__ . '/geography2024.json';
...
function loadQuestionData($jsonFile) {
    ...
    if (!$data) {
        ...
        echo json_encode(['error' => 'Invalid JSON format or empty file in ' . basename($jsonFile) . '.']);
        exit();
    }
    return $data;
}

// Helper function to flatten the question structure for consistent handling
function flattenQuestions($data, $includeDetails = false) {
    $questions = [];
    
    if (isset($data['sections']) && is_array($data['sections'])) {
        // Handle structured format (sections)
        foreach ($data['sections'] as $section) {
            foreach ($section['questions'] as $question) {
                $optionsArray = [];
                if (isset($question['options']) && is_array($question['options'])) {
                    if (array_keys($question['options']) !== range(0, count($question['options']) - 1)) {
                ...
                if (!$includeDetails) {
                    ...
                    // FIX 1: Ensure correctAnswer is cast to string AND forced to UPPERCASE.
                    if (isset($question['correctAnswer'])) {
                         $question['correctAnswer'] = strtoupper((string)$question['correctAnswer']);
                    }
                }
                
                $question['sectionName'] = $section['sectionName'] ?? 'General';
                $question['sectionId'] = $section['sectionId'] ?? 1;
                $questions[] = $question;
            }
        }
    } elseif (is_array($data)) {
         // Handle a flat array of questions
         foreach ($data as $index => $question) {
             $question['sectionName'] = $question['sectionName'] ?? 'General Geography';
             $question['sectionId'] = $question['sectionId'] ?? 1;
             $question['questionId'] = $question['questionId'] ?? ($index + 1);
        ...
        http_response_code(500);
        echo json_encode(['error' => 'Geography JSON structure is unexpected.']);
        exit();
    }
...
if ($action === 'submit') {
    ...
    // Create question map for score lookup
    $questionMap = []; // Keyed by unique questionId
    $indexedQuestions = []; // Array of questions in sequential order (matches front-end)
    $allQuestionsWithDetails = flattenQuestions($data, true);
    
    foreach ($allQuestionsWithDetails as $question) {
    ...
    // Loop through user answers which are keyed by sequential question number (1, 2, 3...)
    foreach ($answers as $qNum => $answer) {
        
        // Use the sequential number to get the correct question object (and its unique ID)
        if (isset($indexedQuestions[$qNum - 1])) { 
            $qObj = $indexedQuestions[$qNum - 1]; 
            $qId = $qObj['questionId']; 
            
            // Use the unique ID to get the correct question details from the robust map
            if (isset($questionMap[$qId])) {
                $correctQuestion = $questionMap[$qId];

                // **CRITICAL FIX:** Check if 'correctAnswer' exists before accessing it to prevent the PHP Warning.
                if (isset($correctQuestion['correctAnswer'])) {
                    
                    // Ensure strict string comparison using UPPERCASE
                    $submittedAnswer = strtoupper((string)$answer);
                    $correctAnswer = strtoupper((string)$correctQuestion['correctAnswer']);
    ...
    exit(); // IMPORTANT: Stops execution before the HTML starts
}
...
<head>
    ...
    <title>Vector Learn — Geography CBT</title>
    <style>
...
<?php include 'topnavbar.php'; ?>
    <div class="container">
        ...
        <div class="title">
            <h1>Vector Learn — Geography 2024</h1>
...
<script>
    ...
    async function loadQuestions() {
        try {
            ...
            if (data.success && data.questions.length > 0) {
                ...
                if (current > allQuestions.length) current = allQuestions.length;
                ...
                showError('Failed to load questions or no questions found: ' + (data.error || 'Check JSON file path/format.'));
            }
        } catch (error) {
    ...
    function renderQuestion() {
        ...
        // --- Navigation button logic FIX (for Next button) ---
    ...
    function saveAnswer() {
        ...
        // Ensure the saved answer is a string, and normalize to uppercase (client-side backup)
        const selectedRadio = form.answer.value ? form.answer.value.toUpperCase() : null; 
        
        if (selectedRadio) {
            answers[current] = selectedRadio;
        } else {
    ...
    async function submitAnswers() {
        ...
        try {
            ...
            // FIX: Read raw response text first for better error handling
            const resultText = await response.text(); 
            
            if (!response.ok) {
                throw new Error(`Server returned HTTP status ${response.status}. Full response starts: ${resultText.substring(0, 100)}...`);
            }

            try {
                const result = JSON.parse(resultText); // Try to parse the text as JSON
                if (result.success) {
                ...
                // Catch JSON parsing errors and display unexpected server output (the Warnings)
                const debugOutput = htmlEscape(resultText.substring(0, 400));
                showError('Error submitting answers: JSON parse failed. Unexpected Server Output (Start): <br><pre style="white-space: pre-wrap; word-break: break-all; max-height: 100px; overflow-y: auto; background: #fff59d;">' + debugOutput + '</pre><br>**ACTION REQUIRED:** This usually means your PHP is printing Warnings/Errors before the JSON. Check your error logs or ensure PHP error reporting is off for production, or fix the underlying logic errors.');
            }
        } catch (error) {
    ...
    function displayExplanations() {
        ...
        explanationData.forEach((qObj, index) => {
            ...
            // The answer is already saved in uppercase by saveAnswer()
            const userAnswer = answers[questionNumber]; 
            const isCorrect = userAnswer === qObj.correctAnswer;
            
            html += '<div class="question-container">';
            html += '<div class="question-header">';
            html += '<span class="question-id">Q' + questionNumber + ' — ' + (isCorrect ? 'Correct ✅' : 'Incorrect ❌') + '</span>';
            html += '<span class="question-section">' + htmlEscape(qObj.sectionName) + '</span>';
            html += '</div>';
             // Check for image
            if (qObj.image) {
            ...
            qObj.options.forEach(opt => {
                ...
                const normalizedOptionId = opt.optionId.toUpperCase();
                // Ensure correct answer is also normalized for display check
                const normalizedCorrectAnswer = qObj.correctAnswer ? qObj.correctAnswer.toUpperCase() : null;

                if (normalizedOptionId === normalizedCorrectAnswer) {
                    labelClass = 'correct-answer-label';
                } else if (normalizedOptionId === userAnswer && normalizedOptionId !== normalizedCorrectAnswer) {
