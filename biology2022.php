<?php
// BIOLOGY 2022 CBT - Standalone PHP Page
// Data source: biology2022.json

// --- Configuration ---
$jsonFile = __DIR__ . '/biology2022.json'; // CHANGED: Updated JSON file name
$subjectTitle = 'Biology 2022'; // CHANGED: Updated subject title

// --- Functions to process the JSON file ---

/**
 * Safely loads and decodes JSON data, accessing the nested 'questions' array.
...
function load_json_data($file) {
    ...
    // ADJUSTED: Access the nested 'questions' array.
    if (!isset($data['questions']) || !is_array($data['questions']) || empty($data['questions'])) {
        $json_error = json_last_error_msg();
        http_response_code(500);
        echo json_encode(['error' => 'Invalid JSON format. Expected a nested "questions" array. PHP JSON Error: ' . $json_error]);
        exit();
    }
    return $data['questions']; // Return the nested array
}

/**
 * Prepares questions for the exam view (removes correct answers and explanation).
 * Mapped 'text' to 'question', 'image_reference' to 'image', and 'topic' to 'sectionName'.
...
function prepare_exam_questions($raw_questions) {
    ...
    foreach ($raw_questions as $q) {
        if (!isset($q['id']) || !isset($q['text']) || !isset($q['options'])) continue; 
        
        $question = [
            'questionId' => (int) $q['id'],
            'question' => $q['text'], 
            'image' => $q['image_reference'] ?? null, 
            'table' => $q['table'] ?? null, 
            'options' => [],
            'sectionName' => $q['topic'] ?? 'Biology', // Changed default
...
<script>
    ...
    function renderTable(tableData) {
        ...
        headers.forEach(key => {
            ...
            if (key.includes('price') || key.includes('value') || key.includes('cost')) {
                // Example currency formatting for Nigerian Naira (₦) as seen in the JSON sample
    ...
    function renderQuestion() {
        ...
        // NOTE: Your image paths use 'Image:biology2022/imageX.jpg'. You might need to adjust the path here 
        // if your images are hosted elsewhere or if 'Image:' needs to be stripped.
        if (qObj.image) {
            let imgSrc = qObj.image;
            if (imgSrc.startsWith('Image:')) {
                // Assuming 'Image:' prefix needs to be handled client-side if it's not a valid URL prefix
                imgSrc = imgSrc.replace('Image:', ''); 
            }
            html += '<img src="' + htmlEscape(imgSrc) + '" alt="Question Diagram"/>';
        }
        if (qObj.table) {
        ...
        // FIX: Explicitly enable 'Next' button if it's not the last question
    ...
    async function loadQuestions() {
        try {
            ...
            if (data.success && data.questions.length > 0) {
                ...
                showError(data.error || 'Failed to load questions or question bank is empty. Check if biology2022.json exists and is valid.');
            }
        } catch (error) {
