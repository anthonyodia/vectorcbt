<?php
// AGRIC 2022 CBT - Standalone PHP Page
// Data source: agric2022.json (expected to be a top-level array of questions)

// --- Configuration ---
// IMPORTANT: Changed to use the correct file name based on user's request.
$jsonFile = __DIR__ . '/agric2022.json';
$subjectTitle = 'Agric 2022'; 
...
function load_json_data($file) {
    ...
    // Crucial Check: Ensure the data is a non-empty array (top-level structure)
    if (!is_array($data) || empty($data)) {
        // Log or print the specific JSON error for debugging
        $json_error = json_last_error_msg();
        http_response_code(500);
        // Updated error message to reflect the current file
        echo json_encode(['error' => 'Invalid JSON format. Expected a top-level array of questions from agric2022.json. PHP JSON Error: ' . $json_error]);
        exit();
    }
    return $data; 
}
...
function prepare_exam_questions($raw_questions) {
    ...
    foreach ($raw_questions as $q) {
        ...
        $question = [
            'questionId' => (int) $q['id'],
            'question' => $q['question'],
            // Image key renamed to 'image' for simplicity, assuming it holds the path
            'image' => $q['image_key'] ?? null, 
            'table' => $q['table'] ?? null,
...
<script>
    ...
    // Note: The provided JSON did not use the 'table' key, but the function is kept for completeness.
    ...
    function renderQuestion() {
        ...
        // Image tag directly uses the provided path
        if (qObj.image) {
    ...
    async function loadQuestions() {
        try {
            // PHP self reference uses agric2022.json now
            ...
            if (data.success && data.questions.length > 0) {
                ...
                // Error message updated to reflect the correct JSON file
                showError(data.error || 'Failed to load questions or question bank is empty. Check if agric2022.json exists and is valid.');
            }
        } catch (error) {
