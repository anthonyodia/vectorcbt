<?php
// CHEMISTRY 2023 CBT - Standalone PHP Page
// Data source: chemistry2023.json (expected to be a top-level array of questions or an object with a 'questions' key)

// --- Configuration ---
// NOTE: This script requires a file named 'chemistry2023.json' in the same directory.
$jsonFile = __DIR__ . '/chemistry2023.json'; 
$subjectTitle = 'WAEC Chemistry 2023'; 
...
function load_json_data($file) {
    if (!file_exists($file)) {
        ...
        echo "Error: Question file not found: " . basename($file) . ". Ensure it is in the same directory.";
        exit();
    }
...
function prepare_exam_questions($raw_questions) {
    ...
    foreach ($raw_questions as $q) {
        ...
        $question = [
            ...
            'table' => $q['table'] ?? null,
            'options' => [],
            'sectionName' => $q['sectionName'] ?? 'General Chemistry', // Updated Default section
...
<?php include 'topnavbar.php'; ?>
    <div class="container">
        ...
        <div class="title">
            ...
            <p>Mastering stoichiometry and reactions, one question at a time.</p>
        </div>
...
<script>
    ...
    async function loadQuestions() {
        try {
            // Note: The PHP_SELF variable ensures the script calls itself to trigger the API actions
            ...
            if (data.success && data.questions.length > 0) {
                ...
                showError(data.error || 'Failed to load questions or question bank is empty. Check if chemistry2023.json exists and is valid.');
            }
        } catch (error) {
