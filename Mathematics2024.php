...
// Mathematics 2024 CBT - Standalone PHP Page
$jsonFile = __DIR__ . '/mathematics2024.json';
...
function loadJsonData($file) {
    ...
    foreach ($questionsArray as $q) {
        $q['sectionName'] = $q['sectionName'] ?? "Core Mathematics";
        $q['sectionId'] = $q['sectionId'] ?? "MATH";
...
// --- ACTION 3: Submit Answers & Record in DB ---
if ($action === 'submit') {
    header('Content-Type: application/json');
    ...
    foreach ($allQuestions as $index => $question) {
        ...
        $stmt = $pdo->prepare("
            ...
            (question_number, subject, question_text, user_answer, correct_answer, is_correct, explanation) 
            VALUES (:qNum, :subject, :question_text, :user_answer, :correct_answer, :is_correct, :explanation)
        ");
        $stmt->execute([
            ':qNum' => "Q$qNum",
            ':subject' => $question['sectionName'] ?? 'Core Mathematics',
...
// --- HTML & JS remain exactly as your original code ---
...
<head>
    ...
    <title>Vector Learn — Mathematics CBT 2024</title>
    <style>
...
<?php include 'header.php'; ?>
    <div class="container">
        ...
        <div class="title">
            <h1>Vector Learn — Mathematics 2024</h1>
        ...
        <div class="question-container" id="q-container">
            <div class="error">
                **CRITICAL JSON ERROR:** The quiz cannot load.
                ...
                <p>Please check your <code>mathematics2024.json</code> file for syntax issues.</p>
            </div>
        </div>
        <?php else: ?>
