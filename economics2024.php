...
// *** CRITICAL CHANGE: Updated file name to economics2024.json ***
$jsonFile = __DIR__ . '/economics2024.json';
...
if ($action === 'get_questions') {
    ...
    // *** ROBUST PARSING LOGIC: Checks for both bare array and object with 'questions' key ***
    if (isset($decodedData['questions']) && is_array($decodedData['questions'])) {
        $fullData = $decodedData['questions']; // Handles the 2023 object structure
    } elseif (is_array($decodedData)) {
        $fullData = $decodedData; // Handles the 2024 bare array structure
    } else {
        http_response_code(500);
        echo json_encode(['error' => 'Invalid JSON format. Expected a bare array or an object with a "questions" key.']);
        exit();
    }
    ...
    $questions = [];
    // Section name remains Economics
...
<head>
    ...
    <title>Vector Learn — Economics</title>
    <style>
...
<?php include 'header.php'; ?>
    <div class="container">
        ...
        <div id="section-display">
            <div class="section-divider">Subject: Economics</div>
        </div>
