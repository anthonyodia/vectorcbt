<?php
// LITERATURE 2025 CBT - Standalone PHP Page
// Data source: literature2025.json

// --- Configuration ---
$jsonFile = __DIR__ . '/literature2025.json'; 
$subjectTitle = 'Literature in English 2025'; 
...
function prepare_exam_questions($raw_questions) {
    ...
    foreach ($raw_questions as $q) {
        ...
        $question = [
            ...
            'table' => $q['table'] ?? null,
            'options' => [],
            'sectionName' => $q['sectionName'] ?? 'General Literature',
...
if (in_array($action, ['get_questions', 'get_explanations', 'submit'])) {
    ...
    if ($action === 'submit') {
        ...
        $score = 0;
        $correct_answers_map = [];
        foreach ($raw_questions as $q) { if (isset($q['id']) && isset($q['answer'])) { $correct_answers_map[(int)$q['id']] = $q['answer']; } }
...
<?php include 'header.php'; ?>
    ...
    <div class="container">
        ...
        <div class="title">
            ...
            <p>Analyzing the beauty of prose, poetry, and drama.</p>
        </div>
        ...
        <div class="question-container" id="q-container"><p style="text-align:center;">Loading literature questions...</p></div>
...
<script>
    ...
    let allQuestions = [];
    let answers = {};
    let totalSeconds = 3000; // 50 minutes for 50 questions
    let timerInterval;
    ...
    function renderQuestion() {
        ...
        document.getElementById('section-display').innerHTML = `<div class="section-divider">Current Focus: ${htmlEscape(qObj.sectionName)}</div>`;
        const qc = document.getElementById('q-container');
        let html = `<div class="question-header"><span class="question-id">Question ${qObj.questionId}</span><span class="question-section">Section ID: ${qObj.sectionId}</span></div>`;
        ...
        if (current === allQuestions.length) {
            ...
            if (!btnSubmit) {
                ...
                btnSubmit.textContent = 'Submit Literature Exam';
    ...
    function displayResults(result) {
        ...
        document.getElementById('info-box').innerHTML = 'Submission Complete!';

        const resView = document.createElement('div');
        resView.className = 'question-container';
        resView.style.textAlign = 'center';
        resView.innerHTML = `<div style="font-size: 32px; font-weight: bold;">Literature Result</div><div style="font-size: 48px; color: #007aff; margin: 20px 0;">${result.score}/${result.total}</div><div style="font-size: 24px; color: #43e97b; margin-bottom: 20px;">${result.percentage}%</div><div class="nav-button-group"><button class="nav-btn explanation-btn" style="background:#007aff" onclick="viewExplanations()">Review Explanations</button><button class="nav-btn" onclick="location.reload()">Try Again</button></div>`;
    ...
    async function viewExplanations() {
        ...
        data.questions.forEach((q, i) => {
            ...
            html += `<div class="question-container"><div class="question-header"><span class="question-id">Q${i+1} — ${isCorrect ? 'Correct ✅' : 'Incorrect ❌'}</span></div><div class="question-text">${htmlEscape(q.question)}</div>${q.options.map(opt => {
            ...
            }).join('')}<div class="explanation-box"><strong>Insight:</strong> <p>${htmlEscape(q.explanation)}</p></div></div>`;
        });

        html += `<div class="nav-button-group"><button class="nav-btn export-btn" onclick="exportPerformance()">Export JSON Performance</button><button class="nav-btn" style="background:#6c757d" onclick="location.reload()">Finish</button></div></div>`;
    ...
    function exportPerformance() {
        ...
        const exportData = explanationData.map(q => ({
            ...
            question: q.question,
            options: q.options,
            userAnswer: answers[q.questionId] || "Not Answered",
            correctAnswer: q.correctAnswer,
            status: answers[q.questionId] === q.correctAnswer ? "PASS" : "FAIL",
            insight: q.explanation
        }));
        const blob = new Blob([JSON.stringify(exportData, null, 2)], {type: 'application/json'});
        const a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = 'Literature2025_Performance.json';
        a.click();
    }
