<?php
// Government 2022 CBT - Standalone PHP Page
// This file handles both loading and serving the Government CBT

// *** FILE NAME UPDATED TO GOVERMENT2022.JSON ***
$jsonFile = __DIR__ . '/goverment2022.json';

// Check if action is requested via AJAX
$action = $_GET['action'] ?? $_POST['action'] ?? null;

// Helper function to process the nested options format from the new JSON structure
function extractOptions($optionsArray) {
    $options = [];
    if (!is_array($optionsArray)) return $options;

    foreach ($optionsArray as $optObj) {
        if (is_array($optObj) && count($optObj) === 1) {
            $key = key($optObj);     // e.g., 'A'
            $value = current($optObj); // e.g., 'socialism'
            $options[] = ['optionId' => $key, 'text' => $value];
        }
    }
    return $options;
}
...
if ($action === 'get_questions') {
    ...
    if (!file_exists($jsonFile)) {
        ...
        echo json_encode(['error' => 'Question file not found: goverment2022.json']);
        exit();
    }
    ...
    // *** ACCESSING 'cbt_quiz' ARRAY ***
    $questionsData = $data['cbt_quiz'] ?? [];

    if (!$questionsData) {
        http_response_code(500);
        echo json_encode(['error' => 'Invalid JSON format or missing "cbt_quiz" array in goverment2022.json']);
        exit();
    }
    ...
    foreach ($questionsData as $question) {
        ...
        $q = [
            'questionId' => $question['question_number'], // Using 'question_number' as ID
            'question' => $question['question'],
            'sectionName' => 'Government', // Defaulting section name
            'sectionId' => 'Government_2022', 
            // Transform the options format
            'options' => extractOptions($question['options']) 
        ];
...
if ($action === 'get_explanations') {
    ...
    // Typo fix: file_get_contents is correct
    $jsonContent = file_get_contents($jsonFile); 
    $data = json_decode($jsonContent, true);
    ...
    foreach ($questionsData as $question) {
        ...
        // No 'data' field to handle in this Government JSON, but keeping the loop structure simple.
...
if ($action === 'submit') {
    ...
    $questionMap = [];
    foreach ($questionsData as $question) {
        // Use 'question_number' from JSON as the key, and 'correct_answer' as the correct answer
        $questionMap[$question['question_number']] = $question['correct_answer'];
    }
    ...
    foreach ($answers as $qId => $answer) {
        // qId is already an integer or string that matches question_number
...
<head>
    ...
    <title>Vector Learn — Government 2022 CBT</title> 
    <style>
        ...
        /* Style for data list inside explanation box (used for Commerce, kept for consistency) */
        .explanation-box ul {
            list-style: none;
            padding: 0;
            margin: 10px 0;
...
<?php include 'header.php'; ?>
    ...
    <div class="container">
        ...
        <div class="title">
            <h1>Vector Learn — Government 2022</h1>
...
<script>
    ...
    async function loadQuestions() {
        try {
            ...
            if (data.success && data.questions.length > 0) {
                ...
                showError('Failed to load questions. ' + (data.error || 'Check goverment2022.json format/existence.'));
            }
        } catch (error) {
    ...
    function renderQuestion() {
        ...
        qObj.options.forEach(opt => {
            ...
            // Use questionId for answer storage, which matches the JSON 'question_number'
        ...
        if (!explanationData) {
              try {
                  ...
                  if (data.success) {
                      ...
                      // Original error loading text is preserved to catch JSON parse errors
                      explanationContainer.innerHTML = '<div class="error">Error loading explanations: JSON.parse: unexpected character at line 1 column 1 of the JSON data</div>';
                  }
              } catch (error) {
    ...
    function displayExplanations() {
        ...
        explanationData.forEach((qObj, index) => {
            ...
            if (qObj.explanation) {
                // Since this JSON doesn't have a 'data' field, we don't need the list processing, 
                // we just print the explanation directly.
