<?php
// CHEMISTRY 2025 CBT - Vector Learn Style
// Data source: chemistry2025.json

// --- Configuration ---
$jsonFile = __DIR__ . '/chemistry2025.json'; 
$subjectTitle = 'Chemistry 2025'; 
...
function prepare_exam_questions($raw_questions) {
    ...
    foreach ($raw_questions as $q) {
        ...
        $question = [
            ...
            'table' => $q['table'] ?? null,
            'options' => [],
            'sectionName' => $q['sectionName'] ?? 'General Chemistry',
...
<?php include 'topnavbar.php'; ?>
    <div class="container">
        ...
        <div class="title">
            ...
            <p>Mastering the principles of atoms, reactions, and matter.</p>
        </div>
        ...
        <div class="question-container" id="q-container"><p style="text-align:center;">Loading chemistry questions...</p></div>
...
<script>
    ...
    function renderQuestion() {
        ...
        document.getElementById('section-display').innerHTML = `<div class="section-divider">Current Module: ${htmlEscape(qObj.sectionName)}</div>`;
        ...
        if (current === allQuestions.length) {
            ...
            if (!btnSubmit) {
                ...
                btnSubmit.textContent = 'Finalize Chemistry Exam';
    ...
    function displayResults(result) {
        ...
        resView.innerHTML = `<div style="font-size: 32px; font-weight: bold;">Chemistry Result</div><div style="font-size: 48px; color: #007aff; margin: 20px 0;">${result.score}/${result.total}</div><div style="font-size: 24px; color: #43e97b; margin-bottom: 20px;">${result.percentage}%</div><div class="nav-button-group"><button class="nav-btn explanation-btn" style="background:#007aff" onclick="viewExplanations()">Review Explanations</button><button class="nav-btn" onclick="location.reload()">Try Again</button></div>`;
    ...
    async function viewExplanations() {
        ...
        html += `<div class="nav-button-group"><button class="nav-btn export-btn" onclick="exportPerformance()">Export Forensic JSON</button><button class="nav-btn" style="background:#6c757d" onclick="location.reload()">Finish</button></div></div>`;
    ...
    function exportPerformance() {
        ...
        a.download = 'Chemistry2025_Performance.json';
        a.click();
    }
