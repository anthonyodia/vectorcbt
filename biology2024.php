<?php
// BIOLOGY 2024 CBT - Standalone PHP Page
// Data source: biology2024.json (expected to be a top-level array of questions or an object with a 'questions' key)

// --- Configuration ---
// NOTE: This script requires a file named 'biology2024.json' in the same directory.
$jsonFile = __DIR__ . '/biology2024.json'; 
$subjectTitle = 'Biology 2024'; 
...
function load_json_data($file) {
    ...
    // Check for array structure: prioritize array inside 'questions' key (as in the provided sample)
...
function prepare_exam_questions($raw_questions) {
    ...
    foreach ($raw_questions as $q) {
        ...
        $question = [
            ...
            'table' => $q['table'] ?? null,
            'options' => [],
            'sectionName' => $q['sectionName'] ?? 'General Biology', // Default section
    ...
    async function loadQuestions() {
        try {
            ...
            if (data.success && data.questions.length > 0) {
                ...
                showError(data.error || 'Failed to load questions or question bank is empty. Check if biology2024.json exists and is valid.');
            }
        } catch (error) {
