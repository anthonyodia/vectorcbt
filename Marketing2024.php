...
if ($action === 'get_questions') {
    ...
    if (!file_exists($jsonFile)) {
        ...
        echo json_encode(['error' => 'Question file (marketing2024.json) not found']);
        exit();
    }
    ...
    foreach ($data as $question) {
        // Adapt to the user's simpler JSON structure which is a flat array of questions
        unset($question['answer']);
        unset($question['explanation']);
        // Add default section info for compatibility with the frontend template
        $question['sectionName'] = "Marketing Fundamentals";
        $question['sectionId'] = "MKTG";
        // Rename 'options' structure for template compatibility if necessary, or just use as is
        $questions[] = $question;
    }
...
if ($action === 'get_explanations') {
    ...
    foreach ($data as $question) {
        // Adapt to the user's simpler JSON structure
        $question['sectionName'] = "Marketing Fundamentals";
        $question['sectionId'] = "MKTG";
        $question['correctAnswer'] = $question['answer']; // Map 'answer' to 'correctAnswer'
        // Assuming 'options' array is kept as-is, which the JS handles
...
if ($action === 'submit') {
    ...
    // Total questions is the count of the array
    $questionsArray = $data;
    $score = 0;
    $total = count($questionsArray);
    
    // Iterate through all questions to check the corresponding user answer
    foreach ($questionsArray as $qIndex => $question) {
        $qNumber = $qIndex + 1; // 1-based index
        $correctAnswer = $question['answer']; // The correct option key (e.g., 'C')
        $userAnswer = $answers[$qNumber] ?? null; // The user's selected option key
...
<?php include 'topnavbar.php'; ?>
    <div class="container">
        ...
        <div class="title">
            <h1>Vector Learn — Marketing 2024</h1>
...
<script>
    ...
    function renderQuestion() {
        ...
        // Adapt to use the simple 'id' as the display number
        html += '<span class="question-id">Q' + qObj.id + '</span>';
        html += '<span class="question-section">' + htmlEscape(qObj.sectionName) + '</span>';
        html += '</div>';
        
        // Handle image display if an image path is present (none in this JSON, but good to keep)
         if (qObj.image) {
        ...
        for (const [optionId, text] of Object.entries(qObj.options)) {
            ...
            if (answers[current] === optionId) html += 'checked';
            html += ' />';
    ...
    // *** THE CRITICAL FIX: EVENT DELEGATION ***
    // This is the solution to the broken 'Next' button issue.
    // It attaches the listener to the static parent, '#nav-buttons', and detects the target element's ID.
    ...
    async function submitAnswers() {
        ...
        try {
            const response = await fetch('<?php echo $_SERVER['PHP_SELF']; ?>?action=submit', {
                ...
                // We send the 1-based index keys used on the client side
