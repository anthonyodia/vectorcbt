<?php
// accounting.php - Vector Learn CBT Page (Server-Side Logic)

// Define the JSON file name
$jsonFile = __DIR__ . '/accounting.json';
...
function flattenQuestions($data, $includeAnswers = false) {
    ...
    foreach ($source_questions as $question) {
        ...
        $q = [
            ...
            'data_table' => $question['data_table'] ?? null,
            'options' => $options, // Now guaranteed to be an array
        ];

        if ($includeAnswers) {
            // Check 'answer' key (used in Accounting JSON) then 'correctAnswer'
...
if ($action === 'get_questions') {
    ...
    echo json_encode([
        'success' => true,
        'examName' => $data['exam'] ?? 'Accounting CBT',
        'subjectName' => $data['subject'] ?? 'Accounting CBT',
...
<head>
    ...
    <style>
        /* CSS omitted for brevity but included in the original provided code */
        body {
        ...
        /* Table Styling for Accounting Data */
        .question-data-table {
            width: 100%;
...
<?php include 'header.php'; ?>
    <div class="container">
        ...
        <div class="title">
            <h1>Vector Learn — <span id="exam-title">Accounting CBT</span></h1>
    ...
    async function loadQuestions() {
        try {
            ...
            if (data.success && data.questions.length > 0) {
                ...
                showError('Failed to load questions. Check that **accounting.json** exists, is valid, and contains questions. (Server responded: ' + JSON.stringify(data) + ')');
            }
        } catch (error) {
