...
// *** CRITICAL CHANGE: Updated file name to English2023.json ***
$jsonFile = __DIR__ . '/English2023.json';
...
if ($action === 'get_questions') {
    ...
    // *** ROBUST PARSING LOGIC: Handles the bare array structure ***
    if (is_array($decodedData)) {
        $fullData = $decodedData; 
    } else {
        http_response_code(500);
        echo json_encode(['error' => 'Invalid JSON format. Expected a bare array of questions.']);
        exit();
    }
    ...
    foreach ($fullData as $questionData) {
        $question = [
            // Note: Using 'number' from JSON for questionId, as 'id' is missing/varied
            'questionId' => $questionData['number'] ?? null, 
            'instruction' => $questionData['instruction'] ?? '', // Added instruction field
            // Combine instruction and question text for display clarity
            'question' => ($questionData['instruction'] ?? '') . (isset($questionData['question']) ? ' ' . $questionData['question'] : ''),
            'options' => [],
...
<head>
    ...
    <title>Vector Learn — English CBT Demo (2023 Exam)</title>
    <style>
...
<script>
    ...
    // Increased duration for 91 questions
    let duration = parseInt(urlParams.get('duration')) || 75; 
    ...
    function renderQuestion() {
        ...
        // Use questionId (which is the question 'number') for display
        html += '<span class="question-id">Q' + qObj.questionId + '</span>'; 
        html += '<span class="question-section">' + qObj.sectionName + '</span>';
        html += '</div>';
        
        // Display the combined instruction and question text
        html += `<div class="question-text">${htmlEscape(qObj.question)}</div>`; 
        
        // Check for and display image if URL exists (though unlikely for English grammar)
        if (qObj.imageUrl) {
    ...
    function renderNav() {
        ...
        for (let i = 1; i <= allQuestions.length; i++) {
            ...
            // Use questionId for answers object lookup
    ...
    function saveAnswer() {
        ...
        // Store answer using questionId (which is the 'number' field from JSON)
