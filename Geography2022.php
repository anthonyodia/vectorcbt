...
// GEOGRAPHY 2022 CBT - Standalone PHP Page

// Data source: geography2022.json


// --- Configuration ---

$jsonFile = __DIR__ . '/geography2022.json';

$subjectTitle = 'Geography 2022'; 
 ...
 * Safely loads and decodes JSON data, supporting a top-level array structure.
...
function load_json_data($file) {
    ...
    if (!file_exists($file)) {
        ...
        echo json_encode(['error' => 'Question file not found: ' . basename($file) . '. Ensure the JSON file exists in the same directory.']);
    ...
    if (!is_array($data) || empty($data)) {
        ...
        echo json_encode(['error' => 'Invalid JSON format. Expected a top-level array of questions. PHP JSON Error: ' . $json_error]);
...
function prepare_exam_questions($raw_questions) {
    ...
    foreach ($raw_questions as $q) {
        ...
        $question = [
            ...
            // Using 'image_path' from the JSON structure
            ...
            'sectionName' => $q['sectionName'] ?? 'General Geography', 
...
function prepare_explanation_questions($raw_questions) {
    ...
    foreach ($raw_questions as $q) {
        ...
        $question = [
            ...
            // Using 'correct_answer' from the JSON structure
...
if (in_array($action, ['get_questions', 'get_explanations', 'submit'])) {
    ...
    if ($action === 'submit') {
        ...
        foreach ($raw_questions as $question) {
            ...
            if (isset($question['id']) && isset($question['correct_answer'])) { 

                $correct_answers_map[(int)$question['id']] = $question['correct_answer'];
...
<head>
    ...
    <style>
        ...
        /* --- CORRECTION FOR 1/3 IMAGE SIZE (Approx. 33% Width) --- */

        .question-text img {

            /* Sets the image width to 33% of the container width. */
            width: 500px; 
            height: 300px; /* Maintains the image's aspect ratio */
            
            display: block;
            margin: 15px auto; /* Centers the smaller image */
            border-radius: 8px;
...
<script>
    ...
    function renderQuestion() {
        ...
        if (currentSection !== qObj.sectionId) {
            ...
            document.getElementById('section-display').innerHTML = 
                ...
                '<div class="section-divider">Section: ' + htmlEscape(qObj.sectionName ?? 'General Geography') + '</div>'; // Added default section name
        ...
        html += '<span class="question-section">' + htmlEscape(qObj.sectionName ?? 'General Geography') + '</span>'; // Added default section name
        ...
        if (qObj.image) {
            // Start of Click-to-Zoom link
            html += '<a href="' + htmlEscape(qObj.image) + '" target="_blank" title="Click to view full-size map">';
            html += '<img src="' + htmlEscape(qObj.image) + '" alt="Question Diagram"/>';
            html += '</a>';
            // Added instruction for the user
            html += '<p style="text-align:center; font-size:12px; color:#555;">(Click map to zoom in a new window)</p>';
        }
    ...
    function saveAnswer() {
        ...
        // Ensure form.answer exists and has a value property (radio buttons return the checked value)
    ...
    function updateTimer() {
        ...
        if (totalSeconds <= 0) {
            ...
            // Give a slight delay before submitting to ensure timer shows 00:00
    ...
    async function loadQuestions() {
        ...
        try {
            ...
            if (data.success && data.questions && data.questions.length > 0) {
                ...
                // Set default section name for all questions if missing in JSON (for safety)

                allQuestions = allQuestions.map(q => ({...q, sectionName: q.sectionName || 'General Geography'}));
                ...
                showError(data.error || 'Failed to load questions or question bank is empty. Check if geography2022.json exists and is valid.');

            }

        } catch (error) {

            showError('Network Error: Could not connect to the server to load questions. Check your server environment.');
    ...
    function displayExplanations() {
        ...
        explanationData.forEach((qObj) => {
            ...
            html += '<span class="question-section">' + htmlEscape(qObj.sectionName ?? 'General Geography') + '</span>';
