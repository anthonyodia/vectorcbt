<?php
// Mathematics 2023 CBT - Standalone PHP Page
// This file handles both loading and serving the CBT

// CRITICAL FIX 1: Point to the correct JSON data file.
$jsonFile = __DIR__ . '/mathematics2023.json';
...
if ($action === 'get_questions') {
    ...
    if (!file_exists($jsonFile)) {
        ...
        echo json_encode(['error' => 'Question file (mathematics2023.json) not found']);
        exit();
    }
    ...
    // Adapt to the user's JSON structure: reading the 'questions' array
    $questionsArray = $data['questions'] ?? [];
    
    if (!$questionsArray) {
        http_response_code(500);
        echo json_encode(['error' => 'Invalid JSON format or missing "questions" array.']);
        exit();
    }
    ...
    foreach ($questionsArray as $question) {
        ...
        // Add default section info for compatibility with the frontend template
        $question['sectionName'] = "Core Mathematics";
        $question['sectionId'] = "MATH";
        $questions[] = $question;
    }
...
if ($action === 'get_explanations') {
    ...
    foreach ($questionsArray as $question) {
        // Add default section info for display
        $question['sectionName'] = "Core Mathematics";
        $question['sectionId'] = "MATH";
        $question['correctAnswer'] = $question['answer']; // Map 'answer' to 'correctAnswer'
...
// --- ACTION 3: Submit Answers and Calculate Score (CORRECTED) ---
if ($action === 'submit') {
    header('Content-Type: application/json');
    
    $input = json_decode(file_get_contents('php://input'), true);
    $answers = $input['answers'] ?? []; // Array of user answers keyed by question ID (1, 2, 3...)
    ...
    // Corrected logic: Iterate through all questions from the JSON
    foreach ($questionsArray as $question) {
        // Use the question's 'id' (e.g., 1, 2, 3...) to access the user's answer.
        // This 'id' matches the key sent by the JavaScript client.
        $qId = $question['id']; 
        
        $correctAnswer = $question['answer']; // The correct option key (e.g., 'C')
        // Retrieve the user's selected option using the question ID as the key
...
<?php include 'header.php'; ?>
    ...
    <div class="container">
        ...
        <div class="title">
            <h1>Vector Learn — Mathematics 2023</h1>
...
<script>
    ...
    let allQuestions = [];
    // Answers are stored using the question ID (1, 2, 3...) as the key.
    let answers = {};
    ...
    function renderQuestion() {
        ...
        // Uses the simple section name added in the PHP logic
        ...
        // Use the question's 'id' property for display
        html += '<span class="question-id">Q' + qObj.id + '</span>'; 
        html += '<span class="question-section">' + htmlEscape(qObj.sectionName) + '</span>';
        html += '</div>';
        
        // Handle image display if an image path is present
         if (qObj.image) {
        ...
        // Iterate over the options object (A, B, C, D)
        for (const [optionId, text] of Object.entries(qObj.options)) {
            html += '<label class="option-label">';
            html += '<input type="radio" name="answer" value="' + optionId + '" ';
            // Use the question's ID to check if an answer has been selected
            if (answers[qObj.id] === optionId) html += 'checked'; 
            html += ' />';
            html += '<span>' + optionId + '. ' + htmlEscape(text) + '</span>';
            html += '</label>';
        }
    ...
    function renderNav() {
        ...
        for (let i = 1; i <= allQuestions.length; i++) {
            // Get the ID of the question at index (i-1)
            const qId = allQuestions[i - 1].id; 
            const answered = answers[qId] ? 'answered' : ''; // Check if answer[qId] exists
    ...
    function updateNavButtons() {
        ...
        if (current === allQuestions.length) {
            ...
            if (!existingSubmit) {
                ...
                // Ensure existing Submit button is visible
                existingSubmit.style.display = 'inline-flex';
            }
        } else {
            // Not Last Question: Show Next, Hide/Remove Submit
            btnNext.style.display = 'inline-flex';
            btnNext.disabled = false; // Ensure it's enabled 
            if (existingSubmit) {
                // Remove the submit button if it's no longer the last question
    ...
    // CRITICAL FIX: Event Delegation for navigation and submission
    document.getElementById('nav-buttons').addEventListener('click', (event) => {
        const target = event.target;
        
        if (target.id === 'btn-prev' && current > 1) {
            if (!target.disabled) {
                navigate(current - 1);
            }
        } else if (target.id === 'btn-next' && current < allQuestions.length) {
            if (!target.disabled) {
                navigate(current + 1);
            }
    ...
    function saveAnswer() {
        ...
        // Use the actual question ID for saving the answer
        const qObj = allQuestions[current - 1];
        if (qObj && selected) answers[qObj.id] = selected; 
        
        renderNav(); // Update navigation bar status immediately
    }
    ...
    async function submitAnswers() {
        ...
        try {
            const response = await fetch('<?php echo $_SERVER['PHP_SELF']; ?>?action=submit', {
                ...
                // Send the answers object keyed by question ID
    ...
    function displayExplanations() {
        ...
        explanationData.forEach((qObj) => {
            const questionId = qObj.id; // Use the actual question ID
            const userAnswer = answers[questionId]; 
            const isCorrect = userAnswer === qObj.correctAnswer;
            
            html += '<div class="question-container">';
            html += '<div class="question-header">';
            html += '<span class="question-id">Q' + qObj.id + ' — ' + (isCorrect ? 'Correct ✅' : 'Incorrect ❌') + '</span>';
            html += '<span class="question-section">' + htmlEscape(qObj.sectionName) + '</span>';
            html += '</div>';
            ...
            for (const [optionId, text] of Object.entries(qObj.options)) {
                let labelClass = '';
                if (optionId === qObj.correctAnswer) {
                    labelClass = 'correct-answer-label';
                } else if (optionId === userAnswer && optionId !== qObj.correctAnswer) {
            ...
            // Use the 'explanation' field from the JSON
            if (qObj.explanation) {
    ...
    function displayResults(result) {
        ...
        // Remove existing result box if any, to prevent duplication
