...
<head>
    ...
    <title>Vector — WAEC Commerce CBT</title>
    <style>
...
<?php include 'topnavbar.php'; ?>
    <div class="container">
        ...
        <div class="title">
            <h1>Vector — <span id="exam-title">WAEC Commerce</span></h1>
...
<script>
    // --- Configuration ---
    // UPDATED: Target the correct JSON file name
    const jsonFile = 'commerce2024.json';
    ...
    let questions = [];
    // Answers stored by question 'id' key from the JSON
    let answers = {};
    let totalQuestions = 0;
    let timerInterval;
    // FIXED: Set the exam title variable to the simple format
    let examTitle = 'WAEC Commerce'; 
        ...
        try {
            ...
            if (data && typeof data === 'object') {
                ...
                if (content && Array.isArray(content.questions)) {
                    ...
                    // FIXED: Keep the simplified title, ignoring any complex title in JSON
                    examTitle = 'WAEC Commerce'; 
                } else {
                    throw new Error("JSON format invalid. Expected {ROOT_KEY: {questions: []}} structure.");
                }
            } else {
                throw new Error("JSON format invalid. Expected a root object.");
            }
            // *** END ADAPTATION ***

            totalQuestions = questions.length;
            // Apply the simplified title
    ...
    function renderQuestion() {
        ...
        // FIXED: Use the simplified section name
        html += `<span class="question-section">Commerce</span>`; 
        html += '</div>';

        // Assuming question text is in qObj.question based on previous structure
        ...
        // This logic handles dynamically replacing the 'Next' button with 'Submit Exam' on the last question
        const nav = document.getElementById('nav-buttons');
        let existingSubmit = document.getElementById('btn-submit');

        if (current === totalQuestions) {
            if (!existingSubmit) {
                // Hide Next, create and append Submit
                ...
                // Append Submit button after Prev button
                nav.appendChild(submitBtn);
                existingSubmit = submitBtn; // Update reference
            } else {
                 existingSubmit.style.display = 'inline-flex';
                 btnNext.style.display = 'none';
            }
        } else {
            // Re-show Next, remove Submit if it exists
            btnNext.style.display = 'inline-flex';
            btnNext.disabled = (current >= totalQuestions); // Re-enable if needed
            if (existingSubmit) existingSubmit.remove();

             // Ensure the Next button click event is attached when it is visible
            btnNext.onclick = () => { saveAnswer(); if (current < totalQuestions) navigate(current + 1); };
        }
    ...
    // Note: btn-next click handler is managed within renderQuestion due to submit button replacement.
